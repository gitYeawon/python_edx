...
def str_analysis(user) :
    
    while True :
    
        if user == "" :
            print("word or integer ")
            user = input("I will analyze you: ")
    
        if user.isalpha() :
            return user + "is all alphabetical characters!"
            break
            
        elif user.isdigit():
            if int(user) > 99:
                return user + " is a pretty big number"
                break
            else:
                return user + " is smaller number than expected"
                break
            
                
user = input("I will analyze you: ")
print(str_analysis(user))
